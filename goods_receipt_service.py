...
from app.services.purchase_order_service import get_purchase_order_by_id, get_purchase_order_items_by_po_id
...
def ensure_goods_receipt_items_exist(goods_receipt_id):
    """
    Erstellt automatisch Wareneingangspositionen aus den Bestellpositionen,
    falls für diesen Wareneingang noch keine Positionen existieren.

    Fachliche Idee:
    Ein Wareneingang basiert auf einer Bestellung.
    Die Bestellpositionen werden automatisch als Prüfpositionen übernommen.
    Der Benutzer trägt danach nur noch gelieferte Menge und Condition ein.
    """
    ...
    existing_items = get_items_by_goods_receipt_id(goods_receipt_id)

    # Wenn Positionen schon existieren, nichts doppelt erzeugen
    if len(existing_items) > 0:
        return True, "Wareneingangspositionen existieren bereits."

    po_id = goods_receipt["PO_ID"]
    po_items = get_purchase_order_items_by_po_id(po_id)

    if len(po_items) == 0:
        return False, "Für diese Bestellung wurden keine Bestellpositionen gefunden."

    if len(goods_receipt_items) == 0:
        next_id = 9001
    else:
        next_id = max(item["GOODS_RECEIPT_ITEM_ID"] for item in goods_receipt_items) + 1

    for po_item in po_items:
        new_item = {
            "GOODS_RECEIPT_ITEM_ID": next_id,
            "GOODS_RECEIPT_ID": int(goods_receipt_id),
            "PO_ID": int(po_id),
            "PO_ITEM_ID": po_item["PO_ITEM_ID"],
            "ARTICLE": po_item["ARTICLE"],
            "ORDERED_QTY": po_item["ORDERED_QTY"],
            "RECEIVED_QTY": 0,
            "CONDITION_ID": 408,
            "CONDITION_NAME": "PRUEFUNG AUSSTEHEND",
        }

        goods_receipt_items.append(new_item)
        next_id += 1

    return True, "Wareneingangspositionen wurden automatisch aus der Bestellung übernommen."

def update_goods_receipt_item(
    ...
    """
    Bearbeitet eine automatisch erzeugte Wareneingangsposition.

    Der Benutzer trägt vor allem ein:
    - gelieferte Menge
    - Condition-ID
    - optional beschädigt / Falschlieferung
    """

    for item in goods_receipt_items:
        if item["GOODS_RECEIPT_ITEM_ID"] == int(goods_receipt_item_id):
            ...
            item["RECEIVED_QTY"] = float(received_qty)
            item["CONDITION_ID"] = condition_id
            item["CONDITION_NAME"] = get_condition_name(condition_id)

            return True, "Wareneingangsposition wurde aktualisiert."

    return False, "Wareneingangsposition wurde nicht gefunden."